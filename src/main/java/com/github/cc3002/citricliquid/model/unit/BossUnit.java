...
public class BossUnit extends AbstractUnit {
    ...
    public void attack(IUnit unit) {
        if (this.isKO()){throw new AssertionError("This Boss Unit is Dead, Cannot Attack" );}
        else {
            int fullAtk = this.roll() + this.atk;
            unit.receiveBossAttack(this, fullAtk);
        }
    }
    ...
    public void receivePlayerAttack(Player player, int fullAtk) {
        if (this.isKO()){throw new AssertionError("This BossUnit is Dead, Cannot Be attacked" );}
        else {
            if (this.roll() < 4) {
     ...
     * and executes the corresponding method.
     * After, it check's if the Boss unit is KO and modifies the stars and wins of the attacking wild unit.
